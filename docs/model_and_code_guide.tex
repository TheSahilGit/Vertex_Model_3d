...
\definecolor{crowdcolor}{RGB}{225,100,90}
...
\begin{tikzpicture}[scale=1.15, every node/.style={font=\small}]
  ...
  \node[above=2pt, text=apicalblue!65!black] at (A0) {\textbf{apical face}};
  \node[below right=-2pt and 2pt, text=basalorange!65!black] at (B3) {\textbf{basal face}};
  ...
  \draw[<-, thick, black!55] ($(A5)!0.6!(B5)$) -- ++(1.0,0.15) node[right, align=left, font=\scriptsize, text=black!55] {\textbf{lateral face}\\ (one per edge)};
\end{tikzpicture}
...
\section{Topological transitions: T1, T2, cell division, and T4}
...
checks for all four, periodically (every \texttt{it\_topology\_check}
steps for T1/T2/T4, every \texttt{it\_division\_check} steps for
...
crossed. All four transitions follow one shared design rule, worth
...
\subsection{T4: crowding-induced live-cell extrusion}
\label{sec:T4}

\textbf{Biology.} Cells can also leave a tissue while still perfectly
healthy, simply because their neighbours are pressing in on them too
hard. This is a well-documented, distinct phenomenon from apoptotic T2
removal: overcrowded epithelia actively extrude live cells to relieve
mechanical stress and keep cell density under control (the
crowding-extrusion literature -- e.g. Eisenhoffer and Rosenblatt's
review work, and Marinari \emph{et al.}'s study of overcrowding-driven
delamination -- is the usual reference point). T4 is this code's model
of that process. It is optional and off by default
(\texttt{T4\_enable}, \texttt{para.in}).

\textbf{Why T4 cannot just reuse T2 outright.} T2
(Section~\ref{sec:T2}) removes a cell by collapsing \emph{every} one of
its corners onto a single point -- fine for a triangle, but a violent,
all-at-once rewrite if applied directly to a healthy hexagon: all six
of its neighbours would suddenly meet at one degenerate point in a
single step. Crowding extrusion needs something gentler: the crowded
cell's neighbours should \textbf{merge faces one at a time} as it
shrinks -- gaining new adjacencies gradually -- not all snap together
at once. That gentler process turns out to already exist in this
code, just not previously used this way.

\textbf{The trigger.} A cell counts as crowded once its volume has been
squeezed well below its own target,
\begin{equation}
  V_i < \texttt{V\_extrusion\_threshold} \times V_i^0,
  \label{eq:T4-trigger}
\end{equation}
the exact volume-based mirror of Section~\ref{sec:division}'s own
growth criterion ($V_i > \texttt{V\_division\_threshold}\times V_i^0$
triggers a split): reusing a quantity (\texttt{cells(ic)\%V\_last}
compared against \texttt{cells(ic)\%V0}) every cell already tracks, no
...
\begin{tikzpicture}[scale=0.85, every node/.style={font=\small}]
  ...
  \def\shiftx{0.55}
  \def\shifty{-1.65}
  \foreach \i in {0,...,5}{ \coordinate (A\i) at ({\Rap*cos(60*\i+90)},{\Rap*sin(60*\i+90)}); }
  \foreach \i in {0,...,5}{ \coordinate (B\i) at ({\shiftx+\Rbas*cos(60*\i+90)},{\shifty+\Rbas*sin(60*\i+90)}); }
  \foreach \i in {0,...,5}{
    \pgfmathtruncatemacro{\j}{mod(\i+1,6)}
    \fill[lateralgray, opacity=0.55] (A\i.center) -- (A\j.center) -- (B\j.center) -- (B\i.center) -- cycle;
    \draw[thin, black!50] (A\i.center) -- (B\i.center);
  }
  \fill[crowdcolor, opacity=0.55] (B0) \foreach \i in {1,...,5}{ -- (B\i) } -- cycle;
  \draw[thick] (B0) \foreach \i in {1,...,5}{ -- (B\i) } -- cycle;
  \fill[crowdcolor, opacity=0.8] (A0) \foreach \i in {1,...,5}{ -- (A\i) } -- cycle;
  \draw[thick] (A0) \foreach \i in {1,...,5}{ -- (A\i) } -- cycle;
  % inward "squeeze" arrows from three alternating directions
  \foreach \ang in {30, 150, 270} {
    \draw[-{Latex[length=2.5mm]}, thick, black!60] ({1.55*1.85*cos(\ang)},{1.55*1.85*sin(\ang)}) -- ({1.55*1.05*cos(\ang)},{1.55*1.05*sin(\ang)});
  }
  \node[right, font=\scriptsize, align=left] at (2.3,-1.4) {squeezed by\\crowding\\neighbours:\\$V_{\mathrm{last}} < $\\ \texttt{V\_extrusion\_}\\\texttt{threshold}$\times V^0$};
\end{tikzpicture}
\caption{A crowded cell, drawn as the same kind of genuine 3D vertex
column as Fig.~\ref{fig:one-cell} (solid apical shell, faint basal
shell): its volume has been pushed below its own target by the
neighbours pressing in on it. This is the trigger condition,
Eq.~\eqref{eq:T4-trigger} -- what happens next is described below.}
\label{fig:T4-3d}
\end{figure}

\textbf{The mechanism: repeated T1 flips, then a T2 finish.} Every
\texttt{it\_topology\_check} tick (T4 shares T1/T2's cadence -- no
separate knob), every crowded cell with more than 3 sides has
\emph{one} of its own edges flipped: specifically its own
currently-shortest edge, using \texttt{try\_T1\_edge} -- the very same
primitive ordinary T1 neighbour exchange already uses
(Section~\ref{sec:T1}). Recall exactly what one T1 flip does: the two
cells that were \emph{not} sharing that edge become neighbours across
a brand new wall, while the two that \emph{did} share it each lose a
side (Fig.~\ref{fig:T1}). Doing this, once per tick, to a crowded
cell's own shrinking boundary is precisely ``neighbours merge faces,
one new adjacency at a time''. Once the crowded cell has been whittled
down to 3 sides, one more flip is not meaningful (there is no smaller
polygon to shrink toward), so it is handed directly to
\texttt{do\_T2} -- the very same finishing move ordinary T2 already
uses (Section~\ref{sec:T2}, Fig.~\ref{fig:T2}) -- to complete the
removal.

\begin{figure}[h]
\centering
\begin{tikzpicture}[every node/.style={font=\small}]
  \node[regular polygon, regular polygon sides=6, draw, thick, fill=crowdcolor, fill opacity=0.6, minimum size=2.0cm] (P6) at (0,0) {};
  \node[below=8pt] at (P6.south) {\footnotesize $n=6$};
  \draw[-{Latex[length=3mm]}, thick] (P6.east) -- ++(1.1,0) node[midway, above, font=\scriptsize, align=center] {T1 flip\\(own shortest edge)};

  \node[regular polygon, regular polygon sides=5, draw, thick, fill=crowdcolor, fill opacity=0.65, minimum size=1.7cm] (P5) at (3.3,0) {};
  \node[below=8pt] at (P5.south) {\footnotesize $n=5$};
  \draw[-{Latex[length=3mm]}, thick] (P5.east) -- ++(1.1,0) node[midway, above, font=\scriptsize, align=center] {T1 flip};

  \node[regular polygon, regular polygon sides=4, draw, thick, fill=crowdcolor, fill opacity=0.7, minimum size=1.4cm] (P4) at (6.1,0) {};
  \node[below=8pt] at (P4.south) {\footnotesize $n=4$};
  \draw[-{Latex[length=3mm]}, thick] (P4.east) -- ++(1.1,0) node[midway, above, font=\scriptsize, align=center] {T1 flip};

  \node[regular polygon, regular polygon sides=3, draw, thick, fill=crowdcolor, fill opacity=0.75, minimum size=1.2cm] (P3) at (8.7,0) {};
  \node[below=8pt] at (P3.south) {\footnotesize $n=3$};
  \draw[-{Latex[length=3mm]}, thick] (P3.east) -- ++(1.3,0) node[midway, above, font=\scriptsize, align=center] {T2 collapse\\(\texttt{do\_T2})};

  \fill (11.2,0) circle (2.2pt);
  \node[below=8pt] at (11.2,-0.15) {\footnotesize removed};
\end{tikzpicture}
\caption{The T4 shrinkage sequence for one crowded cell, one
\texttt{it\_topology\_check} tick per arrow: each T1 flip (identical to
Fig.~\ref{fig:T1}, applied to the cell's own shortest edge) removes one
side and merges two of its neighbours across a new wall; once at
$n=3$, \texttt{do\_T2} (identical to Fig.~\ref{fig:T2}) finishes the
removal. Nothing about ``this cell is mid-extrusion'' is remembered
between ticks -- every tick simply re-checks Eq.~\eqref{eq:T4-trigger}
fresh and, if still crowded, takes the next step shown here.}
\label{fig:T4-sequence}
\end{figure}

\textbf{Fully stateless, no side-count cap.} Nothing in
Fig.~\ref{fig:T4-sequence} is remembered from one tick to the next: if
\texttt{try\_T1\_edge} declines the flip this tick (e.g.\ a momentarily
non-trivalent vertex, or a flanking cell already at
\texttt{MAX\_SIDES}), the cell is simply reconsidered from scratch,
with no bookkeeping, at the next tick; if its volume happens to recover
above threshold partway through, it just stops being touched. No new
per-cell field is added anywhere for this. There is also no upper cap
on how large $n$ may start out: every single action taken is one
already-validated T1 flip (or a final \texttt{do\_T2} only once
$n=3$), so however large a crowded cell starts, it simply takes more
ticks -- it can never vanish in one violent step the way a direct
point-collapse of a large polygon would.

\textbf{Direction: which way was it squeezed out?} Real live-cell
extrusion can go either way -- apically (out through the outer
surface) or basally (delaminating toward the lumen) -- and it is worth
recording which, per event. At the moment of the final \texttt{do\_T2}
call, the cell's own two rings each get their \emph{net resultant
radial force} summed from the code's own analytic forces --
\texttt{f\_api}/\texttt{f\_bas}, exactly as left by that step's
\texttt{compute\_forces} (Section~\ref{sec:forces}), with every energy
term already included (volume, apical/basal area and perimeter,
lateral tension). Since the shell is centred on the origin, the radial
direction at vertex $j$ is simply $\hat r_j = \vec r_j / |\vec r_j|$,
so
\begin{equation}
  F^{\mathrm{net}}_{\mathrm{api}} = \sum_{j \,\in\, \text{ring}} \vec f^{\,\mathrm{api}}_j \cdot \hat r^{\,\mathrm{api}}_j,
  \qquad
  F^{\mathrm{net}}_{\mathrm{bas}} = \sum_{j \,\in\, \text{ring}} \vec f^{\,\mathrm{bas}}_j \cdot \hat r^{\,\mathrm{bas}}_j,
  \label{eq:T4-direction-forces}
\end{equation}
summed over that cell's own apical/basal vertex columns. Whichever
ring's net radial force is more \emph{outward} (the larger, i.e.\ the
less negative, of the two) is being pushed out rather than squeezed
in, and is taken as the side the cell is being squeezed out toward.

\begin{physbox}
\textbf{Why the force, and why the more-outward ring -- checked
against a synthetic wedge, not assumed.} An earlier version of this
rule instead compared how compressed each face's \emph{area} is
relative to its own target, $\text{ratio}_{\mathrm{api}}=A_{\mathrm{last}}/A^0$
vs.\ $\text{ratio}_{\mathrm{bas}}=A^{\mathrm{bas}}_{\mathrm{last}}/A^{0,\mathrm{bas}}$,
and took the \emph{less}-compressed face (the larger ratio) as the
exit direction. That rule is physically intuitive and was itself
checked directly against this code's own analytic forces
(\texttt{tetra\_vol\_grad}/\texttt{tri\_area\_grad}, exactly as used in
Section~\ref{sec:forces}) on a synthetic tapered cell: the
lateral-tension term acts like a drawstring running around each ring's
own perimeter, and a \emph{bigger} ring simply has more perimeter for
that drawstring to grip -- so a positive $\Lambda$ cinches the wider
face in harder, exactly the way squeezing a party balloon or a
toothpaste tube near the fat end pushes material out through the
narrow end, not the other way around. The net-resultant-force rule of
Eq.~\eqref{eq:T4-direction-forces} was checked against that very same
synthetic geometry and agrees with the area-ratio rule in every case
tested -- but it is the more fundamental of the two: it reads the
mechanical force straight out of the energy functional, rather than
inferring direction indirectly from a compressed-area proxy, so it is
what this code actually implements.

$F^{\mathrm{net}}_{\mathrm{api}}$ and $F^{\mathrm{net}}_{\mathrm{bas}}$
are ordinary instantaneous quantities, recomputed every single step --
they fluctuate step to step like any other dynamical quantity. A bare
comparison would therefore flicker unpredictably whenever the two are
close, misreporting noise as a real directional bias.
\texttt{T4\_direction\_deadband} (default $0.05$) fixes this the same
way it always did, only now applied to the two net forces' own
\emph{fractional} difference -- a dimensionless quantity in $[-1,1]$,
so the deadband keeps exactly the same meaning and default it had
under the area-ratio rule:
\begin{equation}
  \text{direction} =
  \begin{cases}
    \text{apical}, & \dfrac{F^{\mathrm{net}}_{\mathrm{api}} - F^{\mathrm{net}}_{\mathrm{bas}}}
                          {\lvert F^{\mathrm{net}}_{\mathrm{api}}\rvert + \lvert F^{\mathrm{net}}_{\mathrm{bas}}\rvert}
                          > \texttt{deadband} \\[1.2ex]
    \text{basal}, & \dfrac{F^{\mathrm{net}}_{\mathrm{bas}} - F^{\mathrm{net}}_{\mathrm{api}}}
                          {\lvert F^{\mathrm{net}}_{\mathrm{api}}\rvert + \lvert F^{\mathrm{net}}_{\mathrm{bas}}\rvert}
                          > \texttt{deadband} \\[1.2ex]
    \text{ambiguous}, & \text{otherwise (or both nets $\approx 0$).}
  \end{cases}
\end{equation}
Every completed T4 event increments exactly one of three running
totals -- \texttt{cumulative\_T4\_apical}, \texttt{cumulative\_T4\_basal},
\texttt{cumulative\_T4\_ambiguous} -- alongside the overall
\texttt{cumulative\_T4} count (which always equals the sum of the
three). All four are plain running integers, accumulated in
\texttt{main.f90} exactly like \texttt{cumulative\_T1}/
\texttt{cumulative\_T2} already were, and written out to every
\texttt{diag\_<it>.dat} dump (Section~\ref{sec:io-module-code}) so the
apical/basal/ambiguous split over the whole run can be plotted
afterwards (\texttt{main\_analysis.m}) -- no per-cell history is kept,
only these four whole-tissue totals.
\end{physbox}

\subsection{The defect-cell protocol: reproducing apical extrusion of abnormal cells}
\label{sec:defect}

\textbf{Biology.} A phenomenon distinct from ordinary crowding
(Section~\ref{sec:T4}): when a single cell in an otherwise healthy
epithelium becomes genetically abnormal (e.g.\ an oncogene turns on),
its normal neighbours often recognise it and actively extrude it from
the sheet -- and, characteristically, \emph{apically} (out through the
outer surface), the well-documented ``epithelial defense against
cancer'' (EDAC) response. This is optional and off by default
(\texttt{defect\_enable}, \texttt{para.in}).

\textbf{Mechanism: seed once, then let T4 do the removing.} Once,
right after \texttt{init\_mesh} sets the usual
$V^0,A^0,A^{0,\mathrm{bas}}$ targets, a random
\texttt{defect\_fraction} of the alive cells are marked
\texttt{is\_defect} and given one or more mechanical differences from
the rest of the tissue (the three levers below). No separate removal
mechanism is added: a defect cell becomes crowded and gets extruded
through the exact same T4 machinery (Section~\ref{sec:T4}) as any
other cell -- \texttt{attempt\_T4\_transitions} additionally checks,
for every completed removal, whether the removed cell was one of the
seeded defect cells, and if so tallies its direction too, giving four
extra running totals (\texttt{cumulative\_T4\_defect} and its own
apical/basal/ambiguous breakdown) alongside the ordinary tissue-wide
ones.

\textbf{Three independent levers, each defaulting to a no-op.} All
three can be used alone or in any combination; \texttt{para.in}
documents the exact defaults.
\begin{enumerate}
  \item \texttt{Lambda\_line\_defect} -- gives a defect cell its own
  lateral-tension modulus (\texttt{cells(:)\%lambda\_own},
  Section~\ref{sec:mod-data-code}) instead of the tissue-wide
  \texttt{Lambda\_line}. Since a shared edge's real tension is the
  \emph{average} of its two owning cells' own values
  (Section~\ref{sec:force-code}), this is a strict generalisation:
  with every cell's \texttt{lambda\_own} equal to \texttt{Lambda\_line}
  (the default), \texttt{Force.f90} computes exactly what it always did.
  \item \texttt{defect\_A0\_bas\_scale} -- scales a defect cell's own
  basal-area target ($A_i^{0,\mathrm{bas}}$) by this extra factor,
  representing loss of basal identity/polarity (a documented feature of
  many transformed epithelial cells, independent of adhesion). Unlike
  lever 1, this is a direct, targeted force on the basal face alone,
  with $\Lambda$ and the apical target left untouched for every cell.
  \item \texttt{defect\_boundary\_tension} -- an extra lateral tension
  added \emph{only} on edges connecting a defect cell to a non-defect
  neighbour (a heterotypic interface): the literal actomyosin
  ``purse-string'' mechanism reported for real EDAC. Identifying which
  edges are heterotypic needs to know the neighbour across every edge,
  which \texttt{edge\_cells} (Section~\ref{sec:mod-topology}) could
  answer but at $O(n_{\mathrm{cell}})$ \emph{per edge} -- too slow for
  \texttt{compute\_forces}, called every step. \texttt{Force.f90}
  instead builds a cheap $O(n_{\mathrm{cell}})$-\emph{total}
  vertex-to-cell cache once per call, only when this lever is active
  (\texttt{build\_vertex\_incidence}/\texttt{neighbor\_across\_edge},
  Section~\ref{sec:mod-topology}), and looks the neighbour up in that
  instead.
\end{enumerate}

\begin{physbox}
\textbf{Which lever actually reproduces the real phenomenon -- checked
directly against a running simulation, not assumed.} All three were
tested the same way: seed a handful of cells, watch whether they get
selectively removed and which direction they favour.
\begin{itemize}[leftmargin=1.4em]
  \item Lever 1 needs a \emph{higher} own lateral tension than the rest
  of the tissue to get crowded at all -- a cell's own lambda dominates
  its own self-compression, so a \emph{lower} own lambda relaxes to a
  \emph{larger} volume, the opposite of ``weak and squeezed''. Once
  crowded this way, its exit direction came out mixed/basal-leaning
  across every combination tried -- \emph{not} the real phenomenon's
  apical bias.
  \item Lever 3 behaves almost identically to lever 1 in practice,
  exactly as expected from how sparsely defect cells are normally
  seeded: most of a defect cell's own edges already border a normal
  neighbour, so ``boundary-only'' tension and ``own-cell'' tension act
  on nearly the same edges. Same basal-leaning outcome.
  \item Lever 2 is the one that works: it gives both preferential
  removal (defect cells consistently die off faster, relative to their
  population share, than the rest of the tissue) \emph{and} a genuine
  apical-majority exit direction ($\sim$60\% apical across the
  parameter combinations tried) -- the lever that actually reproduces
  the real phenomenon, and the one documented as recommended in
  \texttt{para.in}.
\end{itemize}
A caveat true of all three: at the mesh sizes exercised while building
this feature, once T4 activity starts at all it has consistently
cascaded to full tissue loss rather than settling into a steady ``a
few cells culled, the rest of the tissue fine forever'' state -- the
statistics above are read from well before the bulk tissue starts
dying, not from a genuinely steady state. No threshold tried avoided
the eventual collapse; a larger mesh or a little thermal noise are the
likeliest fixes if a non-collapsing demonstration is wanted.
\end{physbox}

\subsection{A subtlety when changing mesh resolution: $\Lambda$ and $K_P$ do not scale themselves}
\label{sec:resolution-scaling}
A parameter set validated at one \texttt{N\_subdivision} is not
automatically valid at another -- checked directly, not assumed, while
testing the defect-cell protocol on a finer mesh. \texttt{Lambda\_line}
and \texttt{K\_P}/\texttt{K\_P\_bas} are \emph{bare} terms with no rest
length (Section~\ref{sec:energy}: shorter/less area is always
energetically better, unlike $K_V$/$K_A$/$K_A^{\mathrm{bas}}$, which
resist moving away from a target in \emph{either} direction). A bare
term's restoring force at a given fractional deviation scales
differently with a cell's own linear size than a targeted term's does,
so the \emph{same} modulus value does not produce the same physical
behaviour (the same equilibrium closeness to $V^0$/$A^0$) at a
different mesh resolution.

Concretely: a parameter set that held a 162-cell mesh
($N_{\mathrm{subdivision}}=2$) at $V_{\mathrm{last}}/V^0\approx0.98$
with no defects and no noise left a 642-cell mesh
($N_{\mathrm{subdivision}}=3$) sitting at $V_{\mathrm{last}}/V^0\approx0.87$
under the unchanged \texttt{Lambda\_line} alone -- confirmed by sweeping
\texttt{Lambda\_line} at fixed $K_P$/$K_P^{\mathrm{bas}}$ and watching
that equilibrium ratio move continuously back toward 1 as
\texttt{Lambda\_line} was reduced, while sweeping $K_P$/$K_P^{\mathrm{bas}}$
alone (at the original \texttt{Lambda\_line}) barely moved it at all --
i.e.\ \texttt{Lambda\_line} was the dominant driver in that specific
case, not $K_P$. Rescaling \texttt{Lambda\_line} down (roughly by the
same factor the mesh got finer) restored the original mesh's own
equilibrium ratio. The practical takeaway: after changing
\texttt{N\_subdivision}, re-check that a defect-free, noise-free run
actually settles near $V_{\mathrm{last}}/V^0\approx1$ (e.g.\ by
watching \texttt{lumen\_volume}/\texttt{outer\_area} in
\texttt{diag\_<it>.dat} flatten out, rather than keep drifting, over
the first several thousand steps) before trusting any comparison
against a run at a different resolution.
...
[{Vertex\_Model\_3d/}
  ...
  [{para.in}]
  [{Makefile}]
  [{src/ \textrm{\normalfont\itshape\small\ -- 16 files, the Fortran engine, Sec.~\ref{sec:src-code}}}]
  [{build/}]
  [{data/ \textrm{\normalfont\itshape\small\ -- snap\_/diag\_ output + example/}}]
  [{matlab/ \textrm{\normalfont\itshape\small\ -- 16 files, analysis \& plotting, Sec.~\ref{sec:matlab-code}}}]
  [{videos/}]
  [{docs/}]
]
\end{forest}
...
T2\_transition.f90\\ T4\_transition.f90\\ mod\_defect.f90\\
Cell\_Division.f90\\
io\_module.f90\\ sanity\_checks.f90\\ main.f90
\end{tabular}}
\end{minipage}%
...
cell\_lateral\_area.m \ \ ring\_planes\_for\_location.m\\
...
\label{sec:src-code}
The 16 source files are compiled in a strict dependency order (each
...
$\rightarrow$ \texttt{T4\_transition} $\rightarrow$ \texttt{mod\_defect}
$\rightarrow$ \texttt{Cell\_Division}
$\rightarrow$ \texttt{io\_module} $\rightarrow$ \texttt{sanity\_checks}
$\rightarrow$ \texttt{main}. (\texttt{T4\_transition} must follow both
\texttt{T1\_transition} and \texttt{T2\_transition}, since it calls
into each of them directly -- see below. \texttt{mod\_defect} only
needs \texttt{mod\_parameters}/\texttt{mod\_data}, so its position
right after \texttt{T4\_transition} and before \texttt{Cell\_Division}
is not load-bearing -- it just keeps every T4-adjacent file together.)
...
V\_division\_threshold, V\_extrusion\_threshold,
T4\_direction\_deadband}, and how often to check for each).
...
apical one. \texttt{T4\_direction\_deadband} (Section~\ref{sec:T4})
gets the same sentinel treatment, defaulting to \texttt{0.05} if
omitted; \texttt{T4\_enable} and \texttt{V\_extrusion\_threshold}, by
contrast, are \emph{required} keys with no default at all, exactly like
\texttt{division\_enable}/\texttt{V\_division\_threshold} -- there is
no sensible silent default for ``should this feature be on'' or ``how
compressed counts as crowded''.

The defect-cell protocol (Section~\ref{sec:defect}) adds five more
keys: \texttt{defect\_enable} and \texttt{defect\_fraction} are
required (same reasoning as \texttt{T4\_enable} above);
\texttt{Lambda\_line\_defect} gets the negative-sentinel treatment too,
defaulting to plain \texttt{Lambda\_line} (no adhesion difference) if
omitted; \texttt{defect\_A0\_bas\_scale} and
\texttt{defect\_boundary\_tension} default directly to \texttt{1.0}
and \texttt{0.0} respectively (no sentinel dance needed -- those literal
values already mean ``no effect'', unlike \texttt{Lambda\_line\_defect},
whose off-value depends on whatever \texttt{Lambda\_line} itself is
set to).

\subsection{\texttt{mod\_data.f90} -- the core data structures}
\label{sec:mod-data-code}
...
\texttt{V0, A0, A0\_bas, V\_last, A\_last, A\_bas\_last}). Two more
fields exist purely for the defect-cell protocol
(Section~\ref{sec:defect}): \texttt{lambda\_own}, this cell's own
lateral-tension modulus (every cell gets \texttt{Lambda\_line} by
default at build time, \texttt{mesh\_init.f90}; only a seeded defect
cell's copy is ever changed, and only via lever~1), and
\texttt{is\_defect}, a permanent diagnostic tag set once at seeding
time and otherwise never touched except by \texttt{Cell\_Division.f90}
(a dividing cell's daughters inherit both fields from their parent).
...
\paragraph{Cheap vertex-to-cell cache
(\texttt{build\_vertex\_incidence}, \texttt{neighbor\_across\_edge}).}
Added for the defect-cell protocol's \texttt{defect\_boundary\_tension}
lever (Section~\ref{sec:defect}), which needs to know the neighbouring
cell across every one of a cell's own edges, every single step -- too
often to afford \texttt{edge\_cells}' $O(n_{\mathrm{cell}})$-per-edge
cost. \textbf{\texttt{build\_vertex\_incidence}} loops over every alive
cell once ($O(n_{\mathrm{cell}}\times\texttt{MAX\_SIDES})$ total, the
same order as the rest of \texttt{compute\_forces}) filling
\texttt{vic(1:3,v)}, up to 3 owning cell ids per vertex column $v$
(\texttt{vic\_count(v)} how many). \textbf{\texttt{neighbor\_across\_edge}}
then takes a cell id and its own edge \texttt{(v1,v2)} and returns the
\emph{other} cell owning that edge (0 if not found) by intersecting
\texttt{vic(:,v1)} and \texttt{vic(:,v2)} -- at most $3\times3$
comparisons, effectively $O(1)$ once the cache is built. \texttt{Force.f90}
builds this cache once per \texttt{compute\_forces} call, and only when
\texttt{defect\_boundary\_tension} is actually non-zero, so it costs
...
and flips it if it points inward. Also sets this cell's
\texttt{lambda\_own} to plain \texttt{Lambda\_line} and
\texttt{is\_defect} to \texttt{.false.} here (Section~\ref{sec:mod-data-code}) --
the sane default every cell gets, before \texttt{mod\_defect.f90}'s
\texttt{seed\_defect\_cells} (Section~\ref{sec:defect}) optionally
overrides it for a random subset, later, once the mesh already exists.
\textbf{Calls:} \texttt{cross}, \texttt{norm3}, \texttt{reverse\_ring}.
...
The lateral-tension term (Section~\ref{sec:lateral-walls}) is the one
piece that changed for the defect-cell protocol
(Section~\ref{sec:defect}): every lateral face is now weighted by a
per-\emph{face} value \texttt{w\_lat}, computed fresh for each of a
cell's own edges, instead of the single per-cell \texttt{Lambda\_line}
used everywhere before this feature existed. \texttt{w\_lat} starts as
that cell's own \texttt{cells(ic)\%lambda\_own} (equal to
\texttt{Lambda\_line} for every cell unless lever~1 changed it for a
seeded subset), and, only when \texttt{defect\_boundary\_tension} is
non-zero, gets \texttt{defect\_boundary\_tension} added on top for any
edge whose neighbour (\texttt{neighbor\_across\_edge}, just above) has
a different \texttt{is\_defect} value from this cell's own (lever~3).
Since both of a shared edge's owning cells compute their own
\texttt{w\_lat} independently and each contributes \texttt{w\_lat}$/2$,
a shared edge's real total tension is the \emph{average} of its two
cells' own values (plus the boundary bonus, once, if it applies) --
with no defects seeded this reduces exactly to plain
\texttt{Lambda\_line} for every edge, bit-for-bit identical to the
code before this feature (checked directly, side by side against the
previous commit).
...
\subsection{\texttt{T4\_transition.f90} -- crowding-induced extrusion}
\label{sec:T4-code}
Section~\ref{sec:T4} already derived this file's algorithm in full;
note this module (\texttt{mod\_T4}) \texttt{use}s \texttt{mod\_T1} and
\texttt{mod\_T2} directly (\texttt{only: try\_T1\_edge} and
\texttt{only: do\_T2} respectively) -- it contains no ring-surgery code
of its own.
\paragraph{Scans for and progresses every crowded cell
(\texttt{attempt\_T4\_transitions}).} \textbf{Input:} none directly
(reads \texttt{cells(:)\%V\_last}, \texttt{cells(:)\%V0} and
\texttt{V\_extrusion\_threshold}; does nothing at all if
\texttt{T4\_enable} is false). \textbf{Outputs:} \texttt{n\_flips} (how
many cells had one edge flipped this tick), \texttt{n\_extruded} (how
many completed removals), \texttt{n\_extruded\_apical,
n\_extruded\_basal, n\_extruded\_ambiguous} (that same completed-removal
count broken down by direction, Section~\ref{sec:T4}), and
\texttt{n\_extruded\_defect, n\_extruded\_defect\_apical,
n\_extruded\_defect\_basal, n\_extruded\_defect\_ambiguous} (the same
two breakdowns again, but counting only cells seeded
\texttt{is\_defect} by the defect-cell protocol, Section~\ref{sec:defect}
-- read just before \texttt{do\_T2} clears that slot). \textbf{Calls:}
\texttt{shortest\_own\_edge}, \texttt{try\_T1\_edge} (for a still-large
crowded cell), \texttt{classify\_T4\_direction}, \texttt{do\_T2} (for
one already at 3 sides). Uses a local \texttt{touched(:)} array
exactly the way \texttt{attempt\_T1\_transitions} does, purely to avoid
acting twice in the same pass on a vertex column another flip already
moved this tick -- not persisted between ticks.
\paragraph{Finds a cell's own shortest edge
(\texttt{shortest\_own\_edge}).} \textbf{Input:} a cell id
\texttt{icell}. \textbf{Output:} the two vertex-column ids \texttt{j,
k} spanning its shortest apical edge (both 0 if the ring is degenerate).
\textbf{Calls:} \texttt{norm3}. Measures every edge the same
apical-length way \texttt{attempt\_T1\_transitions} already measures
edges globally -- needed because \texttt{try\_T1\_edge} takes vertex
ids, not a cell id.
\paragraph{Classifies which face a cell was squeezed toward
(\texttt{classify\_T4\_direction}).} \textbf{Input:} a cell id
\texttt{icell} (read just before it is handed to \texttt{do\_T2}, while
\texttt{f\_api}/\texttt{f\_bas} and \texttt{r\_api}/\texttt{r\_bas}
still reflect its final compressed shape and this step's forces).
\textbf{Output:} one of three named integer constants,
\texttt{T4\_DIR\_APICAL}, \texttt{T4\_DIR\_BASAL}, or
\texttt{T4\_DIR\_AMBIGUOUS}, via the deadbanded comparison of the two
rings' net resultant radial force, Eq.~\ref{eq:T4-direction-forces}.
\textbf{Calls:} \texttt{norm3}.

\subsection{\texttt{mod\_defect.f90} -- the defect-cell protocol}
\label{sec:mod-defect-code}
Section~\ref{sec:defect} already derived this module's physics and its
three levers in full. With \texttt{defect\_enable} false (the default)
every routine here is a no-op and \texttt{Force.f90} behaves exactly as
it always did.
\paragraph{Seeds a random subset of cells (\texttt{seed\_defect\_cells}).}
\textbf{Input:} none directly (reads \texttt{defect\_fraction,
Lambda\_line\_defect, defect\_A0\_bas\_scale} from
\texttt{mod\_parameters}; does nothing if \texttt{defect\_enable} is
false). \textbf{Output:} \texttt{n\_seeded}, how many cells were
actually marked. Picks
$\lceil\texttt{defect\_fraction}\times n_{\mathrm{alive}}\rceil$ cells
uniformly at random via a partial Fisher--Yates shuffle of the
alive-cell index list (using the same Fortran random-number stream
already seeded by \texttt{init\_rng}), then for each one sets
\texttt{lambda\_own} to \texttt{Lambda\_line\_defect} (lever~1) and
scales \texttt{A0\_bas} by \texttt{defect\_A0\_bas\_scale} (lever~2)
before finally marking \texttt{is\_defect=.true.} \textbf{Calls:} the
intrinsic \texttt{random\_number}. Called exactly once, from
\texttt{main.f90}, right after the usual $V^0,A^0,A^{0,\mathrm{bas}}$
targets are set and before the very first real time step (lever~3,
\texttt{defect\_boundary\_tension}, needs no per-cell state here at
all -- it is applied live inside \texttt{Force.f90},
Section~\ref{sec:force-code}, keyed purely off \texttt{is\_defect}).
\paragraph{Counts surviving defect cells
(\texttt{count\_alive\_defect\_cells}).} \textbf{Input:} none (reads
global state). \textbf{Output:} how many seeded cells are both
\texttt{alive} and \texttt{is\_defect} right now -- an
\emph{instantaneous} count, unlike the cumulative T4 totals, written to
every \texttt{diag\_<it>.dat} dump (Section~\ref{sec:io-module-code}) so
a run's defect population over time can be read straight off the
diagnostics. \textbf{Calls:} none.
...
Both also inherit the parent's \texttt{lambda\_own} and
\texttt{is\_defect} unchanged (Section~\ref{sec:defect}) -- the parent's
own cell slot keeps its own values automatically (it is simply
relabelled as one daughter), so only the freshly allocated second
daughter needs them copied across explicitly; a defect cell's whole
...
cumulative\_T1, cumulative\_T2, cumulative\_T4,
cumulative\_T4\_apical, cumulative\_T4\_basal,
cumulative\_T4\_ambiguous, n\_defect\_seeded, n\_defect\_alive,
cumulative\_T4\_defect, cumulative\_T4\_defect\_apical,
cumulative\_T4\_defect\_basal, cumulative\_T4\_defect\_ambiguous} --
every scalar diagnostic already computed by the caller
(\texttt{write\_diagnostics\_now} in \texttt{main.f90}; the six defect
fields, Section~\ref{sec:defect}, are purely additive, appended at the
end when that feature was built). \textbf{Output:}
\texttt{data/diag\_<it>.dat}, a small stream/unformatted binary file
with exactly those 19 values, in that order. \textbf{Calls:}
\texttt{append\_diag\_manifest}. Called once per dump, immediately
after \texttt{write\_snapshot}.
...
\begin{enumerate}
  ...
  then one call to \texttt{seed\_defect\_cells} (Section~\ref{sec:defect};
  a no-op unless \texttt{defect\_enable}) -- deliberately \emph{after}
  the targets above are set, so it changes nothing about how
  $V^0,A^0,A^{0,\mathrm{bas}}$ are chosen, only lowers
  \texttt{lambda\_own}/scales \texttt{A0\_bas} for the seeded subset
  afterwards.
  ...
  then \texttt{attempt\_T2\_transitions}, then \texttt{attempt\_T4\_transitions}
  run in that order, with
  ...
  happened (including a T4 flip that did not yet complete a removal).
  Every \texttt{it\_division\_check} steps (if
  ...
  one-line progress summary is printed (with a second line, only when
  \texttt{defect\_enable} and at least one defect cell was extruded that
  tick, breaking that tick's defect-cell extrusions down by direction).
...
including the four running T4 totals
(\texttt{cumulative\_T4\_apical/\_basal/\_ambiguous} and their sum
\texttt{cumulative\_T4}, all accumulated in the time loop above exactly
like \texttt{cumulative\_T1}/\texttt{cumulative\_T2}), the current
defect-cell population (\texttt{count\_alive\_defect\_cells},
Section~\ref{sec:mod-defect-code}, plus the constant
\texttt{n\_defect\_seeded} from start-up), and the same four-way T4
breakdown again restricted to defect cells
(\texttt{cumulative\_T4\_defect} and its own
\texttt{\_apical/\_basal/\_ambiguous}).
\textbf{Calls:} \texttt{compute\_lumen\_volume},
\texttt{count\_alive\_defect\_cells}, the intrinsic \texttt{norm2},
\texttt{write\_diagnostics}.
...
plots energy, lumen volume, outer area, max force, cell count,
cumulative T1, cumulative T2, and cumulative T4 against time in the
first eight panels of a $3\times3$ grid, with the ninth panel showing
T4's apical-ward/basal-ward/ambiguous breakdown as three separate
lines. \textbf{Calls:} \texttt{list\_diagnostics},
\texttt{read\_diagnostics} (once per dump). (The defect-protocol
fields \texttt{read\_diagnostics} also returns,
Section~\ref{sec:defect}, are not yet plotted here -- read directly
from the struct \texttt{D} returned by \texttt{read\_diagnostics} if
you need them meanwhile.)
\paragraph{Tissue/ring videos (\texttt{main\_movie.m}).} Reads a chosen
selection of \texttt{snap\_*.dat} vertex snapshots and renders up to
\emph{three independent videos}, each its own figure/window and each
independently switchable on or off (\texttt{flag\_tissue}/
\texttt{flag\_latitude}/\texttt{flag\_longitude}): the whole-tissue
outside view, a \emph{latitude} ring, and a \emph{longitude} ring
through one chosen cell/location (\texttt{ref\_location} -- a cell
index or an explicit $[x\ y\ z]$ point, resolved once, from the first
selected frame, into two fixed cutting planes reused for every later
frame). ``Latitude''/``longitude'' are defined relative to the z-axis
as the polar axis (Section~\ref{sec:ring-planes} below); with more
...
\texttt{parpool}/\texttt{gcp} are undefined here -- so no
\texttt{parfor}/\texttt{parfeval} is available). \textbf{Calls:}
\texttt{list\_snapshots}, \texttt{select\_snapshots},
\texttt{ring\_planes\_for\_location}, \texttt{make\_tissue\_video},
\texttt{make\_cross\_section\_video} (twice, for the two rings).
...
the 19 scalar fields written by \texttt{write\_diagnostics}
(Section~\ref{sec:io-module-code}): time, energy, lumen\_volume,
outer\_area, max\_force, n\_cells, cumulative\_T1, cumulative\_T2,
cumulative\_T4, cumulative\_T4\_apical, cumulative\_T4\_basal,
cumulative\_T4\_ambiguous, n\_defect\_seeded, n\_defect\_alive,
cumulative\_T4\_defect, cumulative\_T4\_defect\_apical,
cumulative\_T4\_defect\_basal, cumulative\_T4\_defect\_ambiguous.
\textbf{Calls:} none. (The six defect-protocol fields were added after
the rest -- checked directly against a real \texttt{diag\_<it>.dat}: an
earlier version of this file stopped 6 fields short of the actual
record, silently leaving that data unread, since \texttt{fread} simply
reads however many values it is told to and does not complain about
...
none.
\paragraph{Computes each cell's lateral wall area
(\texttt{cell\_lateral\_area}).} \textbf{Input:} \texttt{S}.
\textbf{Output:} one value per alive cell (same order as
\texttt{find(S.cell\_alive)}). Unlike apical/basal area, the lateral
(cell--cell wall) area is not stored in the snapshot at all, so this
computes it directly from \texttt{r\_api}/\texttt{r\_bas} geometry --
the same two-triangles-per-face convention
\texttt{Force.f90} (Section~\ref{sec:force-code}) and
\texttt{plot\_cross\_section} both already use for a lateral quad face.
\textbf{Calls:} none (a plain per-cell, per-edge loop; MATLAB
built-ins only).
...
label (\texttt{clabel}). Ten modes are available:
\texttt{'nsides'}; \texttt{'volume'}/\texttt{'volume\_abs'} (strain
$V/V^0-1$ / absolute $V$); \texttt{'area'}/\texttt{'apical\_area'}
(apical strain / absolute); \texttt{'basal\_area'}/
\texttt{'basal\_area\_abs'} (basal strain / absolute -- the basal face
had no colouring option at all before this); \texttt{'lateral\_area'}
(via \texttt{cell\_lateral\_area}); \texttt{'total\_area'} (apical +
basal + lateral); and \texttt{'shapefactor'}
($S_{\mathrm{total}}/V^{2/3}$, the 3-D analogue of the 2-D vertex
model's dimensionless shape index, minimised by a sphere). The last
three all call \texttt{cell\_lateral\_area}, noticeably more expensive
than the others (real per-cell geometry, not a stored-field lookup);
see \texttt{make\_tissue\_video}/\texttt{make\_cross\_section\_video}
below for how a whole video avoids paying for that twice per frame.
\textbf{Calls:} \texttt{cell\_lateral\_area} (for the last three modes
only). Shared by both \texttt{plot\_tissue\_3d} and
\texttt{plot\_cross\_section} so a single frame and a whole video
always use the exact same colour definition.
\paragraph{Latitude/longitude cutting planes for a location
(\texttt{ring\_planes\_for\_location}).}
\label{sec:ring-planes}
\textbf{Inputs:} \texttt{S} and \texttt{ref} -- either a cell index
(that cell's own apical-face centroid is used) or an explicit
$[x\ y\ z]$ point. \textbf{Outputs:} two plane structs,
\texttt{latPlane} and \texttt{lonPlane} (each a \texttt{.normal}
(3$\times$1 unit vector) and a \texttt{.point} (3$\times$1) -- the
plane $\{\vec r : (\vec r-\texttt{point})\cdot\texttt{normal}=0\}$),
and the resolved reference point itself. Treats the $z$-axis as the
polar axis by convention (this mesh has no intrinsically marked
pole): the latitude plane is horizontal, at the reference's own
height ($\texttt{normal}=\hat z$, $\texttt{point}=(0,0,z_{\mathrm{ref}})$);
the longitude plane is the vertical meridian through the poles and the
reference's own azimuthal angle $\phi=\mathrm{atan2}(y_{\mathrm{ref}},x_{\mathrm{ref}})$
($\texttt{normal}=(-\sin\phi,\cos\phi,0)$, always through the origin).
\textbf{Calls:} none. Meant to be called once (typically on a video's
first frame) and the two planes reused unchanged for every later
frame, exactly like the fixed cutting axis/value this replaced.
\paragraph{Whole-tissue 3D plot (\texttt{plot\_tissue\_3d}).}
\textbf{Inputs:} \texttt{S}, \texttt{colorby}, and name-value pairs:
\texttt{'Figure'}/\texttt{'Axes'} (draw into an existing figure or axes
-- e.g.\ one subplot of a bigger figure -- instead of always owning a
whole figure), \texttt{'CLim'} (a fixed colour range instead of
auto-scaling to this one frame), \texttt{'Title'} (a label prepended
above the usual ``t = ...'' title), \texttt{'ColorMap'} (any built-in
MATLAB colormap name, default \texttt{'parula'}), and \texttt{'CVal'}/
\texttt{'CBLabel'} (an already-computed colour vector/label, to skip
calling \texttt{tissue\_color\_values} again -- see
\texttt{make\_tissue\_video} below). \textbf{Output:} the figure
handle. Draws every alive cell's apical polygon as one shaded,
coloured patch on the outer shell, with a fixed (not auto-fitting)
axis range pinned to $R_{\mathrm{apical}}$ and the view frozen via
\texttt{axis vis3d} (Section~\ref{sec:vis3d-note} below) rather than
just \texttt{CameraViewAngleMode} by hand, a title, and a colourbar.
\textbf{Calls:} \texttt{cell\_faces\_matrix}, \texttt{tissue\_color\_values}
(unless \texttt{'CVal'} was given).
\paragraph{Cross-section ring plot (\texttt{plot\_cross\_section}).}
\textbf{Inputs:} \texttt{S}, a cutting-plane specifier -- either the
legacy \texttt{(cutaxis, cutvalue)} form (\texttt{'x'|'y'|'z'} plus a
scalar, the plane $\{\text{axis}=\text{value}\}$) or a general
\texttt{\{normal, point\}} struct (Section~\ref{sec:ring-planes}, for a
plane at an arbitrary angle, not just axis-aligned) -- and the same
\texttt{'Figure'}/\texttt{'Axes'}/\texttt{'ColorBy'}/\texttt{'CLim'}/
\texttt{'Title'}/\texttt{'ColorMap'}/\texttt{'CVal'}/\texttt{'CBLabel'}
pairs as \texttt{plot\_tissue\_3d}, plus \texttt{'View'} (an explicit
$[\mathrm{az}\ \mathrm{el}]$, e.g.\ one found by hand and read back
with \texttt{[az,el]=view(gca)}, instead of the default automatic
tilt). \textbf{Output:} the figure handle. Finds every cell whose
apical polygon straddles the cutting plane, builds its own explicit
triangle list for every face (apical/basal fans from a synthetic
centroid, lateral quads split into 2 triangles -- deliberately not
handed to MATLAB's own internal triangulation, which was observed to
...
edges separately, and views the result at a slight tilt from face-on
to the cutting plane (a pure face-on view flattens the ring into what
reads as a 2-D strip) unless \texttt{'View'} overrides it, with the
same \texttt{axis vis3d} freeze as \texttt{plot\_tissue\_3d}.
\textbf{Calls:} \texttt{tissue\_color\_values} (unless \texttt{'CVal'}
was given).

\begin{physbox}
\label{sec:vis3d-note}
\textbf{Why \texttt{axis vis3d}, not just freezing the camera zoom by
hand.} Both plotting functions need their view frozen so a video does
not glitch between frames (Section~\ref{sec:code-sanity}'s
observation that \texttt{CameraViewAngleMode} left on \texttt{'auto'}
lets MATLAB silently recompute the zoom on a later redraw). That alone
is not enough for anyone using a saved figure \emph{interactively}: it
leaves \texttt{PlotBoxAspectRatioMode} on \texttt{'auto'}, which
re-fits the plot box's shape to the axes region for whatever the
\emph{current} view direction is -- invisible for one static frame, but
exactly what showed up (checked directly) as the whole scene appearing
to zoom in and out while a user rotated it by hand. \texttt{axis
vis3d}, called once after the axes' final position/limits/view are
set, freezes both properties together and was confirmed (checked
directly) to preserve whatever camera/view was already set rather than
recomputing it.
\end{physbox}

\paragraph{Makes a whole-tissue video (\texttt{make\_tissue\_video}).}
\textbf{Inputs:} a list of snapshot filenames, \texttt{colorby}, an
output filename, a frame rate, and an optional colormap name.
\textbf{Output:} an \texttt{.avi} file (\texttt{'Motion JPEG AVI'}
profile, supported on every platform including Linux, unlike
\texttt{MPEG-4}). Pre-scans every requested snapshot once to fix one
colour range for the whole video (so the colour scale never flickers
frame to frame) \emph{and} caches each frame's own colour values
(\texttt{tissue\_color\_values}) from that same pass, handing them
straight to \texttt{plot\_tissue\_3d} via \texttt{'CVal'}/
\texttt{'CBLabel'} during the render loop instead of recomputing them
-- for a cheap mode (a stored-field lookup) this saves nothing worth
mentioning, but for \texttt{'total\_area'}/\texttt{'shapefactor'}
(which triangulate every cell's lateral walls from raw geometry) it
avoids doing that real computation twice per frame. Also reuses a
single fixed-size figure window, and captures each frame via
...
Exactly mirrors \texttt{make\_tissue\_video} (same colour-value
caching, same colormap-name argument), calling
\texttt{plot\_cross\_section} once per frame instead, with the cutting
plane, an optional title label, and an optional explicit view angle all
fixed once for the whole video. \textbf{Calls:}
...
coordinates, collected into a single vector for brevity:
\begin{equation}
  \vec x \equiv
  \begin{pmatrix} x_1 \\ x_2 \\ x_3 \end{pmatrix}
  \equiv
  \begin{pmatrix} A_i \\ A_i^{\mathrm{bas}} \\ \Sigma_i \end{pmatrix} .
  \label{eq:xvec-def}
\end{equation}
Any energy quadratic in these three quantities can be written
\begin{equation}
...
outer product
\begin{equation}
  M_{\mathrm{surf}} = \vec w\,\vec w^{T}, \qquad
  \vec w \equiv \begin{pmatrix} w_1 \\ w_2 \\ w_3 \end{pmatrix},
  \label{eq:M-surf}
\end{equation}
