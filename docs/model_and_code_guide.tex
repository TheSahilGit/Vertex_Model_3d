...
\begin{tikzpicture}[scale=1.15, every node/.style={font=\small}]
  ...
  \node at ($(A1)!0.5!(B1)$) [right=6pt, font=\scriptsize] {lateral face};
...
The energy used here is a standard, widely-used one, built from four
...
face's perimeter (\texttt{Papi}); $V_i^0$ and $A_i^0$ are its
\emph{target} (preferred) volume and apical area
(\texttt{V0}, \texttt{A0}); and $S_{\mathrm{lat}}$ is the area of one
lateral (cell--cell contact) face. $K_V, K_A, K_P, \Lambda$ are
constants (moduli) set in the parameter file, controlling how strongly
each effect matters.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  a 3D spring.
  \item \emph{Apical area elasticity}, $\tfrac{K_A}{2}(A-A^0)^2$: cells
  tend to maintain a preferred amount of apical surface (related to
  their density in the tissue and to the actin cortex resting there);
  deviating from it also costs energy like a spring.
  \item \emph{Apical perimeter contractility}, $\tfrac{K_P}{2}P^2$:
  the actomyosin ``purse-string'' running around a cell's apical
  perimeter actively contracts, and doing so against the rest of the
  tissue costs energy proportional to (perimeter)$^2$. Note this term
  has \emph{no} target value -- shorter is always energetically better,
  unlike the other two terms which have a preferred value they resist
  moving away from in \emph{either} direction.
  ...
  matters a great deal (Section~\ref{sec:code-sanity} discusses what
...
area and a fixed height), and that the tension term acts on a genuine
3D lateral wall \emph{area} $S_{\mathrm{lat}}$ (not an edge length as it
would in a 2D model). A note on naming: the code calls the modulus
\texttt{Lambda\_line} and this document sometimes still calls the
effect ``line tension'' in asides -- that name is inherited from 2D
vertex models, where the contact between two cells really is a 1D edge
(a line), so the term there is written $\Lambda\cdot(\text{edge
length})$. Here the corresponding contact is a genuine 2D wall, so the
same physical idea becomes $\Lambda\cdot(\text{wall area})$, exactly as
written above -- ``lateral interfacial tension'' is the accurate name
in 3D, and is preferred throughout this document.

\subsection*{Why these four terms, and not some other combination?}
...
three separate face-by-face terms? And having split things up by face,
why is the basal face treated differently from the apical one? And is
the lateral term really a separate ``adhesion'' effect on top of
everything else, or is it the same thing as cell--cell adhesion?
\begin{physbox}
\textbf{Three clarifications.}
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{A cell is not mechanically uniform, so one surface
  tension is not enough.} A real epithelial cell's cortex is
  biologically different at different faces. At the apical face there
  is a contractile actomyosin ``purse-string'' encircling the
  junctional belt -- this is an \emph{active} process (myosin motors
  pulling, whether or not it is energetically favourable) with no
  preferred length, which is exactly what the perimeter term
  $\tfrac{K_P}{2}P^2$ encodes (it only ever wants $P\to 0$). Area
  elasticity $\tfrac{K_A}{2}(A-A^0)^2$ is a completely different,
  \emph{passive} effect: resistance to compressing or stretching the
  apical domain away from a preferred size, which resists changes in
  \emph{either} direction. An active, one-directional pull and a
  passive, two-directional spring cannot be captured by a single
  tension coefficient, so both terms are needed together -- this is
  exactly the reasoning behind the widely used 2D Farhadifar
  \emph{et al.} (2007) vertex energy, of which the apical terms here
  are a direct 3D descendant.
  \item \textbf{Why apical-only, with no basal area/perimeter terms.}
  This is a genuine \emph{simplification of this particular model}, not
  a law of physics -- it could be relaxed. It is made for two reasons:
  first, the apical side is what is actually imaged and tracked in the
  experiments this style of model is built to match (junction markers
  such as E-cadherin or ZO-1 are apical), so the apical terms are the
  ones with direct quantitative grounding; second, in the hollow-shell
  geometries this code targets (Section~\ref{sec:geom-setup}) the basal
  face borders the fluid-filled lumen, not a rigid substrate, so unlike
  a tissue sitting on a real basement membrane there is no obvious
  independent contractile/elastic apparatus to give it its own term
  here. If you wanted to model a tissue whose basal side \emph{does}
  carry its own active/elastic machinery, symmetric basal area and
  perimeter terms could be added to Eq.~\ref{eq:energy} using exactly
  the same fan-triangulation and gradient machinery derived in
  Section~\ref{sec:forces} for the apical ones -- nothing about the
  method restricts it to the apical face, only this particular choice
  of energy does.
...
\end{lstlisting}
Note the basal cap contributes to the cell's \emph{volume} only -- there
is no basal-area or basal-perimeter term in this energy functional (see
the discussion at the end of Section~\ref{sec:energy} for why).
...
perimeter $P_{\mathrm{api}}$ and, for every one of its edges, lateral
...
(arrays \texttt{gV\_A}/\texttt{gV\_B}, \texttt{gArea\_A},
\texttt{gPer\_A}, \texttt{gLat\_A}/\texttt{gLat\_B} in the code). Since
$E$ (Eq.~\ref{eq:energy}) is a sum over cells of simple functions of
these quantities, the chain rule gives, for one cell $i$ and one of its
vertex columns $j$:
\begin{align}
...
likewise for the $K_A$ and $K_P$ terms (elementary calculus on a
quadratic), and where $\Barr_j$ only feeds into $V_i$ and
$S_{\mathrm{lat},i}$ since there is no basal area or perimeter term
(Section~\ref{sec:basal-reverse}). The factor $\tfrac12$ on the
...
$\Lambda\,S_{\mathrm{lat}}$ exactly once, not twice.
...
\section{A known instability: strongly negative lateral tension}
...
($K_V,K_A,K_P,\Lambda$ as \texttt{K\_V,K\_A,K\_P,Lambda\_line}), every
geometric/mesh setting (\texttt{N\_subdivision, R\_apical, R\_basal}),
every dynamics setting (\texttt{zeta\_friction, kBT, dt, n\_steps}), and
every transition threshold (\texttt{L\_T1\_threshold,
A\_T2\_threshold, V\_division\_threshold}, and how often to check for
each). \textbf{Calls:} only Fortran intrinsics (it reads a
\texttt{NAMELIST} block, Fortran's native ``named key=value list''
file format). It also runs a handful of basic sanity checks on the
values themselves (e.g. $R_{\mathrm{basal}}>0$ and
$R_{\mathrm{apical}}>R_{\mathrm{basal}}$, $dt>0$) and stops the program
immediately with a clear message if any of them look nonsensical,
rather than silently running a broken simulation.
...
volume/area and most-recently-computed volume/area
($V^0,A^0,V,A_{\mathrm{api}}$ as \texttt{V0, A0, V\_last, A\_last}).
...
freshly computed volume/area (\texttt{cells(:)\%V\_last,
cells(:)\%A\_last}, later used by \texttt{attempt\_cell\_divisions} to
check the growth threshold). \textbf{Calls:} \texttt{centroid\_n},
...
these mutating calls, so a rejected division touches nothing.
...
every cell's ring, sides, alive flag and volume/area
(target and last-computed). \textbf{Calls:}
...
itself recovers exactly $V=1, A_{\mathrm{api}}=1$ for this cube -- this
directly validates the face-winding/fan-triangulation convention used
throughout \texttt{Force.f90}, not just the raw gradients).
\textbf{Calls:} \texttt{allocate\_data}, \texttt{compute\_forces}.
Called once, at start-up, immediately after the previous check.
...
\begin{enumerate}
  ...
  cell's target volume/area are set as
  $V^0_i=$\texttt{V0\_scale}$\times V_i$,
  $A^0_i=$\texttt{A0\_scale}$\times A_i$ -- i.e. by default
  (\texttt{V0\_scale}=\texttt{A0\_scale}=1) the freshly built mesh
  \emph{is} its own unstrained reference shape, so the volume/area
  elastic energy starts at exactly zero and only the lateral-tension
  term is active at $t=0$. Then \texttt{global\_shell\_checks}.
...
ring/sides/alive flag/volume/area, matching the byte layout documented
