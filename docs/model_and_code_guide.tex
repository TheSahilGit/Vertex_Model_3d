...
[{Vertex\_Model\_3d/}
  ...
  [{para.in}]
  [{Makefile}]
  [{src/ \textrm{\normalfont\itshape\small\ -- 15 files, the Fortran engine, Sec.~\ref{sec:src-code}}}]
  [{build/}]
...
T2\_transition.f90\\ T4\_transition.f90\\ Cell\_Division.f90\\
...
\label{sec:src-code}
The 15 source files are compiled in a strict dependency order (each
...
$\rightarrow$ \texttt{T4\_transition} $\rightarrow$ \texttt{Cell\_Division}
...
into each of them directly -- see below.)
...
many completed removals), and \texttt{n\_extruded\_apical,
n\_extruded\_basal, n\_extruded\_ambiguous} (that same completed-removal
count broken down by direction, Section~\ref{sec:T4}). \textbf{Calls:}
...
cumulative\_T4\_ambiguous} -- every scalar diagnostic already
...
like \texttt{cumulative\_T1}/\texttt{cumulative\_T2}).
...
the 13 scalar fields (time, energy, lumen\_volume, outer\_area,
max\_force, n\_cells, cumulative\_T1, cumulative\_T2,
cumulative\_T4, cumulative\_T4\_apical, cumulative\_T4\_basal,
cumulative\_T4\_ambiguous). \textbf{Calls:} none.
