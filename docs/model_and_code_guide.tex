...
$A_i^{0,\mathrm{bas}}$). Whichever face is proportionally \emph{more}
compressed is taken as the side the cell is being squeezed out toward.

\begin{physbox}
\textbf{Why a deadband, and how the count is kept.} $A_{\mathrm{last}}$
and $A^{\mathrm{bas}}_{\mathrm{last}}$ are ordinary instantaneous
quantities, recomputed by \texttt{compute\_forces} every single step
(Section~\ref{sec:forces}) -- they fluctuate step to step like any
other dynamical quantity. A bare comparison
($\text{ratio}_{\mathrm{api}} < \text{ratio}_{\mathrm{bas}}$, direction
apical, and vice versa) would therefore flicker unpredictably whenever
the two ratios happen to be close, misreporting noise as a real
directional bias. \texttt{T4\_direction\_deadband} (default $0.05$)
fixes this by requiring a \emph{margin}: apical is only recorded when
$\text{ratio}_{\mathrm{api}}$ is more than the deadband \emph{below}
$\text{ratio}_{\mathrm{bas}}$ (and symmetrically for basal); every
event that is closer than that is bucketed as \emph{ambiguous} instead
of being forced arbitrarily to one side or the other:
\begin{equation}
  \text{direction} =
  \begin{cases}
